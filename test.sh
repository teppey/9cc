...
assert 22 'main() { int b; return b = 5 * 6 - 8; }'
assert 14 'main() { int a; int b; a = 3; b = 5 * 6 - 8; return a + b / 2; }'
assert 1 'main() { int foo; return foo = 1; }'
assert 5 'main() { int bar; return bar = 2 + 3; }'
assert 10 'main() { int foo; int bar; foo = 2; bar = 2 + 3; return foo * bar; }'
assert 0 'main() { return 1 == 0; }'
...
assert 0 'main() { int a; a = 0; while (a > 1) a = a + 1; return a; }'
assert 3 'main() { int a; a = 0; while (a < 3) a = a + 1; return a; }'
assert 20 'main() { int a; a = 0; while (a < 3) if (a == 2) a = a * 10; else a = a + 1; return a; }'
assert 1 'main() { for (;;) return 1; }'
assert 1 'main() { int a; for (a = 1;;) return a; }'
assert 2 'main() { for (;0;) return 1; return 2; }'
assert 7 'main() { int i; for (i = 0; i < 7; i = i + 1) 0; return i; }'
assert 11 'main() { int a; a = 0; for (;; a = a + 1) if (a > 10) return a; }'
assert 4 'main() { int a; int i; int j; a = 0; for (i = 0; i < 2; i = i + 1) for (j = 0; j < 2; j = j + 1) a = a + 1; return a; }'
assert 1 'main() { { return 1; } }'
assert 2 'main() { { int a; a = 1; return a + 1; } }'
assert 5 'main() { int a; int b; a = 2; b = 3; if (a > 0) { a = a * a; b = a + 1; } return b; }'
assert 20 'main() { int a; int b; a = b = 0; while (a < 10) { b = b + 1; a = a + 1; } return a + b; }'
assert 1 'main() { {} return 1; }'
